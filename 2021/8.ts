...
const splitAndSort = (str: string) => str
  .split(' ')
  .map(p => p.split('').sort().join(''));

const filterByLength = (arr: string[], length: number) => arr.filter(a => a.length === length);

const filterOutSegments = (str: string, toFilterOut: string) =>
  str
    .split('')
    .filter(a => !toFilterOut.includes(a))
    .join('');
  
const decodePatterns = (patterns: string[]) => {
...
const signals = getInput(8)
  .split('\n')
  .map(line => line.split(' | '))
  .map(([patterns, outputs]) => ({
    patterns: splitAndSort(patterns),
    outputs: splitAndSort(outputs),
  }));
